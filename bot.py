...
def init_db():
    ...
    # Standart tovarlar (bazada yo'q bo'lsa avtomatik qo'shiladi)
    default_products = [
    ...
    for p_id, p_name, p_price, p_cat in default_products:
        cursor.execute("""
            INSERT OR IGNORE INTO inventory (product_id, product_name, price_soem, category) 
            VALUES (?, ?, ?, ?)
        """, (p_id, p_name, p_price, p_cat))
        
    conn.commit()
    conn.close()

# 🗄️ MA'LUMOTLAR OMBORI FUNKSIYALARI
def db_add_user(user_id, username, name):
    conn = sqlite3.connect("store_management.db")
    cursor = conn.cursor()
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    cursor.execute("""
        INSERT OR IGNORE INTO users (user_id, username, name, joined_at) 
        VALUES (?, ?, ?, ?)
...
def db_get_user_orders(user_id):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT order_id, product_name, price_sum, target_username, status, created_at 
...
def db_get_inventory():
    ...
    rows = cursor.fetchall()
    conn.close()
    return {r[0]: {"name": r[1], "price": r[2], "category": r[3]} for r in rows}
...
def auto_backup_database():
    try:
        ...
        logging.error(f"Zaxiralashda xato: {e}")

# 🌐 FLASK VA CORS STRUKTURASI
app = Flask(__name__)
CORS(app, resources={r"/*": {"origins": "*"}})
app.url_map.strict_slashes = False  # Mini App oxiridagi '/' belgisi 404 bermasligi uchun!

# Telegram Webhook bog'lovchisi
...
def getMessage():
    if request.headers.get('content-type') == 'application/json':
        ...
        return "!", 200
    return "Taqiqlangan hudud", 403

@app.route('/')
def home():
    return "Imperator Bot Webhook markazi xavfsiz holatda ishlamoqda!", 200
...
def get_mini_app_user_data():
    ...
    try:
        ...
        if not user_info:
            return jsonify({"success": False, "message": "Avval botga kirib /start bosing!"}), 404
...
def process_mini_app_purchase():
    ...
    try:
        ...
        if success:
            ...
            user_msg = f"🥳 **Xarid muvaffaqiyatli yakunlandi!**\n\n🧾 **Chek raqami:** `{order_id}`\n📦 **Mahsulot:** {p_name}\n💰 **Narxi:** `{price:,} so'm`\n🎯 **Qabul qiluvchi:** `{target}`"
            try: bot.send_message(u_id, user_msg, parse_mode="Markdown")
            except: pass
            
            # Adminga bildirishnoma yuborish
            admin_msg = f"🚨 **YANGI BUYURTMA №{order_id}!**\n\n👤 **Mijoz:** {user_info['name']} ({u_id})\n📦 **Tovar:** `{p_name}`\n💰 **Narxi:** `{price:,} so'm`\n🎯 **Yuborish manzili:** `{target}`"
            kb = InlineKeyboardMarkup()
            kb.add(
                InlineKeyboardButton("👤 Profilini tekshirish", callback_data=f"crm_inspect_{u_id}"),
                InlineKeyboardButton("✅ Bajarildi", callback_data=f"crm_done_{order_id}")
            )
        ...
        return jsonify({"success": False, "message": f"Kutilmagan xato: {str(e)}"}), 500

# 🟢 TELEGRAM INTERFEYSI COMMANDS
...
def super_admin_panel(message):
    ...
    admin_text = f"👑 **IMPERATOR ADMIN PANEL**\n\n👥 Jami mijozlar: `{total_users} ta`\n🚫 Bloklanganlar: `{banned_users} ta`\n📦 Tovar turlari: `{total_products} ta`\n💰 Jami Kassa: `{kassa:,} so'm`"
    
    kb = InlineKeyboardMarkup(row_width=2)
    kb.add(
        InlineKeyboardButton("👤 CRM Qidiruv", callback_data="pnl_user_inspect"),
        InlineKeyboardButton("💰 Balans berish", callback_data="pnl_balance_ctrl"),
        InlineKeyboardButton("📦 Omborxona", callback_data="pnl_inventory_ctrl"),
        InlineKeyboardButton("📢 Reklama tarqatish", callback_data="pnl_broadcast"),
        InlineKeyboardButton("🗄 Ma'lumotlar zaxirasi", callback_data="pnl_backup_now")
    )
    bot.send_message(ADMIN_ID, admin_text, parse_mode="Markdown", reply_markup=kb)

# Admin navigatsiyasi uchun callback boshqaruvchi
@bot.callback_query_handler(func=lambda call: call.data.startswith("pnl_"))
def admin_sub_navigation(call):
    if call.message.chat.id != ADMIN_ID: return
    bot.answer_callback_query(call.id)
    
    if call.data == "pnl_user_inspect":
        msg = bot.send_message(ADMIN_ID, "🔍 Profilni tekshirish uchun foydalanuvchi ID raqamini yoki @username ni yozing:")
        bot.register_next_step_handler(msg, process_crm_user_inspect)
    elif call.data == "pnl_balance_ctrl":
        msg = bot.send_message(ADMIN_ID, "Formatni kiriting: `Foydalanuvchi_ID | Summa` \n(Misol: `7977733681 | 50000` yoki ayrish uchun `-50000`)")
        bot.register_next_step_handler(msg, process_admin_balance_change)
    elif call.data == "pnl_inventory_ctrl":
        kb = InlineKeyboardMarkup(row_width=2)
        kb.add(
        ...
        bot.send_message(ADMIN_ID, "📦 Omborxonani boshqarish bo'limi:", reply_markup=kb)
    elif call.data == "pnl_broadcast":
        msg = bot.send_message(ADMIN_ID, "📢 Barcha foydalanuvchilarga yuborilishi kerak bo'lgan xabarni (Matn, rasm yoki chek) yuboring:")
        bot.register_next_step_handler(msg, process_admin_broadcast)
    elif call.data == "pnl_backup_now":
        auto_backup_database()
        bot.send_message(ADMIN_ID, "✅ Ma'lumotlar bazasi zaxiralanib sizga yuborildi!")

def process_crm_user_inspect(message):
    try:
        input_data = message.text.strip()
        target_id = int(input_data) if input_data.isdigit() else db_get_user_by_username(input_data)
        
        if not target_id:
            bot.send_message(ADMIN_ID, "❌ Kechirasiz, bunday foydalanuvchi ma'lumotlar bazasidan topilmadi!")
            return
        ...
        if orders:
            for o in orders[:10]:
                history_text += f"📦 {o[1]} | `{o[2]:,}` so'm | {o[5]}\n"
        else:
            history_text = "Xaridlar tarixi mavjud emas."
            
        inspect_response = f"👤 **FOYDALANUVCHI PROFILI:**\n🆔 Tizim ID raqami: `{target_id}`\n🏷 Ismi: {user_info['name']}\n⛓ Username: {user_info['username']}\n💰 Balansi: `{user_info['balance']:,} so'm`\n💸 Sarflagan puli: `{user_info['spent_money']:,} so'm`\n📅 Ro'yxatdan o'tdi: {user_info['joined_at']}\n\n🛒 **Oxirgi 10 ta xaridi:**\n{history_text}"
        
        kb = InlineKeyboardMarkup()
        action_type = "unban" if user_info['banned'] else "ban"
        action_text = "🟢 Blokdan ochish" if user_info['banned'] else "🔴 Tizimdan bloklash"
        kb.add(
            InlineKeyboardButton(action_text, callback_data=f"crmaction_{action_type}_{target_id}"),
            InlineKeyboardButton("💰 Balansini o'zgartirish", callback_data=f"crmaction_bal_{target_id}")
        )
        bot.send_message(ADMIN_ID, inspect_response, parse_mode="Markdown", reply_markup=kb)
    except Exception as e:
        bot.send_message(ADMIN_ID, f"❌ Qidirishda xatolik yuz berdi: {e}")
...
def handle_quick_crm_actions(call):
    ...
    if parts[1] == "inspect":
        call.message.text = parts[2]
        process_crm_user_inspect(call.message)
    elif parts[1] == "done":
        # Buyurtmani bajarilgan deb yozuv qo'shish
        try:
            bot.edit_message_caption(
                chat_id=ADMIN_ID, 
                message_id=call.message.message_id, 
                caption=call.message.caption + "\n\n✅ **BU BUYURTMA TO'LIQ ETKAZIB BERILDI!**"
            )
        except:
...
def handle_crm_deep_actions(call):
    ...
    if action == "ban":
        db_set_ban(uid, True)
        bot.send_message(ADMIN_ID, f"✅ Foydalanuvchi (ID: {uid}) muvaffaqiyatli bloklandi.")
    elif action == "unban":
        db_set_ban(uid, False)
        bot.send_message(ADMIN_ID, f"✅ Foydalanuvchi (ID: {uid}) blokdan muvaffaqiyatli ochildi.")
    elif action == "bal":
        msg = bot.send_message(ADMIN_ID, f"💰 ID: `{uid}` bo'lgan foydalanuvchiga beriladigan summani kiriting (Faqat raqamlarda):")
...
def process_direct_balance(message, uid):
    try:
        ...
        bot.send_message(ADMIN_ID, f"✅ Muvaffaqiyatli bajarildi. Foydalanuvchi balansiga {amount:,} so'm qo'shildi.")
        try: bot.send_message(uid, f"💰 Sizning balansingiz administrator tomonidan {amount:,} so'mga o'zgartirildi!")
        except: pass
    except Exception as e:
        bot.send_message(ADMIN_ID, f"❌ Xato! Miqdorni raqamda kiritmadingiz: {e}")
...
def process_admin_balance_change(message):
    try:
        ...
        bot.send_message(ADMIN_ID, f"✅ ID: `{t_id}` balansi muvaffaqiyatli o'zgartirildi.")
        try: bot.send_message(t_id, f"💰 Sizning balansingiz administrator tomonidan {amount:,} so'mga o'zgartirildi!")
        except: pass
    except Exception as e:
        bot.send_message(ADMIN_ID, f"❌ Xato! Format noto'g'ri kiritildi. Misol: `7977733681 | 50000` shaklida bo'lishi kerak. Xato: {e}")
...
def inventory_actions_handler(call):
    ...
    if call.data == "inv_view_list":
        current_inv = db_get_inventory()
        text = "📋 **Omborda hozirda mavjud tovarlar ro'yxati:**\n\n"
        for k, v in current_inv.items():
            text += f"🔑 ID: `{k}` | **{v['name']}** | Narxi: `{v['price']:,}` so'm [Rukn: {v['category']}]\n"
        bot.send_message(ADMIN_ID, text, parse_mode="Markdown")
    elif call.data == "inv_add":
        msg = bot.send_message(ADMIN_ID, "Formatni aniq kiriting: `id | Nomi | Narxi | Rukn` \n\nMisol: `stars_500 | 500 Telegram Stars | 190000 | Stars`")
        bot.register_next_step_handler(msg, process_inventory_add_item)
    elif call.data == "inv_del":
        msg = bot.send_message(ADMIN_ID, "Ombordan o'chirish uchun tovarning qisqa ID raqamini (Masalan: `stars_1`) kiriting:")
...
def process_inventory_add_item(message):
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("""
            INSERT OR REPLACE INTO inventory (product_id, product_name, price_soem, category) 
            VALUES (?, ?, ?, ?)
        """, (p_id, p_name, p_price, p_cat))
        conn.commit()
        conn.close()
...
def process_admin_broadcast(message):
    ...
    bot.send_message(ADMIN_ID, f"✅ Reklama tugadi! Jami {count} ta foydalanuvchiga yuborildi.")

# 🚀 SERVER ISHGA TUSHGANDA WEBHOOKNI BIR MARTA BOG'LASH
init_db()
try:
    bot.remove_webhook()
    render_url = f"https://bot-py-15ln.onrender.com/{BOT_TOKEN}"
    bot.set_webhook(url=render_url, allowed_updates=["message", "callback_query", "web_app_data"])
    logging.info(f"Webhook muvaffaqiyatli o'rnatildi: {render_url}")
except Exception as e:
    logging.error(f"Webhook sozlashda ogohlantirish: {e}")

if __name__ == '__main__':
    # Mahalliy kompyuterda sinash uchun port
